...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------- */
    ...
    Customer.prototype.getCustomerList = function (page) {
        ...
        $.post('/customer.list', p, function (data) {
            if (!!data && data.code == 0) {
    ...
    Customer.prototype.getAllAgentList = function (page) {
        ...
        $.post('/all_agent.list', p, function (data) {
            if (!!data && data.code == 0) {
    ...
    Customer.prototype.searchCustomer = function (key, page) {
        ...
        $.post('/keyword_customer', p, function (data) {
            if (!!data && data.code == 0) {
                That.page_info = data.page_info;
                That.key = key;
                That.keyOn = 1;
    ...
    Customer.prototype.allocationCustomer = function (ids, agentId) {
        var p = {
            CorpIds: ids,
            AgentId: agentId
        }
        $.post('/allocation_customer_agent', p, function (data) {
            if (!!data && data.code == 0) {
    ...
    Customer.prototype.sortByKey = function (p) {
        $.post('/all_customer_sort', p, function (data) {
            if (!!data && data.code == 0) {
                That.page_info = data.page_info;
                That.keyOn = 2;
                That.sort = p;
...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------- */
    ...
    $AllocationBtn.on('click', function () {
        ...
        var agentId = $AgentList.find('option:checked').val();
        CustomerCtl.allocationCustomer(chk_value, Number(agentId))
    })
