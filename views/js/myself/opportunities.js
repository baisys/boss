...
(function (Dom) {
    ...
    Opport.prototype.getOpportList = function (page) {
        ...
        $.post('/opport.list', p, function (data) {
            if (!!data && data.code == 0) {
    ...
    Opport.prototype.searchOpport = function (key, page) {
        ...
        $.post('/keyword_opport', p, function (data) {
            if (!!data && data.code == 0) {
                That.page_info = data.page_info;
                That.key = key;
                That.keyOn = 1;
    ...
    Opport.prototype.deleteOpport = function (ids, status) {
        var p = {
            OpportIds: ids,
            status: status
        }
    ...
    Opport.prototype.sortByKey = function (p) {
        $.post('/opport_sort', p, function (data) {
            if (!!data && data.code == 0) {
                That.page_info = data.page_info;
                That.keyOn = 2;
                That.sort = p;
    ...
    $delOpportBtn.on('click', function () {
        ...
        chk_value.push(+$('#opp_id').val());
        var status = $("#Stage option:checked").val();
        OpportCtl.deleteOpport(chk_value, status)
    });
    ...
    Dom.on('click', '.closeOpportBtn', function () {
        $ModalDelOpport.modal('show');
        $("#opp_id").val($(this).parents('tr').find('.OpportId').data('id'));
    })
