...
func (this *CustomerController) AddComment() {
	...
	comment := &Comment{}
	comment.CorpId = this.GetString("CorpId")
...
func (this *CustomerController) AlertComment() {
	...
	account := this.GetSession(SESSION_USER).(*Account)
	ok := customerModel.AlertComments(commentId, comments, account)
	this.JSonCode(ok)
}
...
func (c *CustomerController) AlertCustomer() {
	...
	customer.Buin, _ = c.GetInt("buin")
	customer.CorpId = c.GetString("Corp_id")
