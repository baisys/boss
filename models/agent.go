...
func (this *AgentModel) GetAgentList(EmployeeId string, page int) (slice []*Agent) {
	...
	query := `select t1.agent_id, t1.name, t1.contacts, t1.mobile, t1.Mail, t1.note, t1.timex, t1.account_id from t_agent t1 
		where t1.enabled =1  order by t1.agent_id desc`
	if page == -1 {
...
func (this *AgentModel) AddAgent(agent *Agent, account *Account) (tag bool) {
	...
	query := `INSERT INTO t_agent(name, contacts, mobile, mail, note, timex, account_id) VALUES (?, ?, ?, ?, ?, ?, ?);`
	res, err := tx.Exec(query, agent.Name, agent.Contacts, agent.Mobile, agent.Mail, agent.Note, agent.Timex, account.AccountId)
	if err != nil {
...
func (this *AgentModel) ShowKeyWorldAgent(KeyWorld string, page int) (slice []*Agent) {
	...
	rows, err := Db.Query(`select t1.agent_id, t1.name, t1.contacts, t1.mobile, t1.Mail, t1.note, t1.timex,t1.account_id from t_agent t1 
	where t1.enabled =1 and t1.name like ?  order by t1.agent_id desc limit ?, ?;`, KeyWorld, offset, PAGESIZE)
	defer rows.Close()
	if err != nil {
		beego.Error("[agent db] db Query err: ", err)
		return
...
func (this *AgentModel) GetCountInfo(KeyWorld string) (int, int) {
	...
	var err error
	if KeyWorld == "" { // 获取所有渠道
		rows, err = Db.Query(`select count(1) from t_agent where enabled=1;`)
	} else {
		KeyWorld = "%" + KeyWorld + "%"
		rows, err = Db.Query(`select count(1) from t_agent where Name like ? and enabled=1;`, KeyWorld)
	}
...
func (this *AgentModel) GetAgentBySort(key string, index, page int) (list []*Agent, page_info map[string]interface{}) {
	...
	rows, err := Db.Query(query, to, PAGESIZE) // rows will be closed in next function [GetAgentListByRows]
	if err != nil {
		beego.Error(err)
		return
	}
