...
func (this *BizModel) GetCustomerNameById(CorpId int) (customerName string) {
	query := `select name from t_corp where corp_id = ?;`
	err := Db.QueryRow(query, CorpId).Scan(&customerName)
	if err != nil {
		beego.Error(err)
	}
...
func (this *BizModel) EmpBizDetail(bizId string, account *Account) (biz *Biz) {
	...
	query := `select t1.biz_id, t1.account_id, t1.corp_id, t1.title, t1.content, t1.amount, 
	t1.status, t1.timex, t1.estimate_time, t1.real_time, t3.name from t_biz t1
    left join t_corp t3 on t1.corp_id = t3.corp_id
    where t1.biz_id = ?;`
	err := Db.QueryRow(query, bizId).Scan(&biz.BizId, &biz.AccountId, &biz.Customer.CorpId, &biz.Title, &biz.Content,
...
func (this *BizModel) FilterBizList(filter *FilterParam, account *Account) (slice []*Biz) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&biz.BizId, &biz.AccountId, &biz.Customer.CorpId, &biz.Title, &biz.Content,
...
// 修改商机对应的流水
func (this *BizModel) AlertCommentsFromC(commentId int, comments string, account *Account) (ok bool) {
	...
	com := make(map[string]interface{}, 0)
	com["备注内容"] = comments
	_comment := NewComments(account, comments, com)
...
func (this *BizModel) InsertBiz(g map[string][]string, account *Account, biz *Biz, tags []string) (tag bool) {
	...
	//使用tx
	// 插入商机基本信息
	query := `insert into t_biz(account_id, corp_id, title, content, amount, status, 
	timex,
	estimate_time, real_time, agent_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`
	res, err := tx.Exec(query, biz.AccountId, biz.Customer.CorpId, biz.Title, biz.Content,
	...
	res, err = tx.Exec(query, biz.Customer.CorpId, account.AccountId, _comment, biz.Timex)
	if err != nil {
		beego.Error(err)
		return
	}
...
func (this *BizModel) GetBizComments(id, _type int, page int) ([]*Comment, error) {
	...
	for rows.Next() {
		c := &Comment{}
		err := rows.Scan(&c.CommentId, &c.CorpId, &c.Committer, &c.Comments, &c.Timex, &c.Type)
		if err != nil {
			beego.Error(err)
			return list, err
		}
...
func (this *BizModel) GetOpportBySort(key string, index, page int, user *Account) (list []*Biz, page_info map[string]interface{}) {
	...
	query = `select t1.biz_id, t1.account_id, t1.corp_id, t1.title, t1.content, t1.amount, 
	t1.status, t1.timex, t1.estimate_time, t1.real_time, t1.enabled, t3.name, t4.NAME from t_biz t1
    	LEFT JOIN t_corp t3 on t1.corp_id = t3.corp_id
	LEFT JOIN t_account t4 on t1.account_id = t4.account_id
...
func (this *BizModel) GetOpportListByRows(list []*Biz, rows *sql.Rows) []*Biz {
	for rows.Next() {
		...
		biz.Customer = &Customer{}
		err := rows.Scan(&biz.BizId, &biz.AccountId, &biz.Customer.CorpId, &biz.Title, &biz.Content,
			&biz.Amount, &biz.Status, &biz.Timex, &biz.EstimateTime, &biz.RealTime, &biz.Enabled, &biz.Customer.EntName, &biz.Account.Name)
		if err != nil {
			beego.Error(err)
			return list
		}
